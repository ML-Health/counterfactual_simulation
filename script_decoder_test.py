...
from core_routines import test, get_data_out
...
if __name__ == "__main__":
    ...
    for net_name in net_names:
        ...
        results_map[seq_net_name] = pd.DataFrame([], index=[i for i in range(10, max_coeff + 1)],
                                             columns=[i for i in range(10, max_coeff + 1)])
        projection_map[seq_net_name] = {}

        for chemo_coeff in [i for i in range(10, max_coeff + 1)]:
            for radio_coeff in [i for i in range(10, max_coeff + 1)]:
                    # Data setup
                    ...
                    last_5_pred,last_5_output  = get_data_out(means, output, test_processed['original_sequence'])
                    
                    mse_last5 = np.sum((last_5_pred - last_5_output) ** 2, axis=0) / last_5_pred.shape[0]
                    mse_last5 = mse_last5.flatten()
                    
                    #import ipdb;ipdb.set_trace()
                    
                    pd.DataFrame(mse_last5).to_csv(os.path.join(RESULTS_FOLDER, "last_5.csv"))
                    ...
                    for proj_idx in range(mse.shape[0]):
                        ...
                        if proj_idx not in projection_map[seq_net_name]:
                            projection_map[seq_net_name][proj_idx] = \
                                pd.DataFrame([], index=[i for i in range(10, max_coeff + 1)],
                                             columns=[i for i in range(10,max_coeff + 1)])
    ...
    for k in projection_map:
        for i in projection_map[k]:
            projection_map[k][i].to_csv(os.path.join(RESULTS_FOLDER, k + "_" + str(i+2) + "static_mse.csv"))
