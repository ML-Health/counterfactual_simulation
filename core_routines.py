...
### This is the function extracting prediction and output of patients of the last 5 time (T - 4, T - 3... T) 
### within testing set. e.g. prediction at T = 56,57,58,59,60
###
### Params 
###
### Prediction: prediction output from the RMSN. A numpy array of dimension (num_patiens x T-1, Horizon, OutputDim)
### Output: output from the simulated cancer dataset. Same dimension as prediction
### Sequence_lengths: length of trajectory for each individual in testing set (1000 patients). 

def get_data_out(prediction, output, sequence_lengths):
        
    num_patients = sequence_lengths.shape[0]

    last_5_pred = []
    last_5_output = []
    horizon = prediction.shape[1] - 1
    
    total_seq2seq_rows = 0    # Counter of trajectory length for each patient
    
    for i in range(num_patients):
        sequence_length = int(sequence_lengths[i])-1   
        pred_data_last = prediction[total_seq2seq_rows + sequence_length - horizon, :, :] # 1, Horizon, OutputDim
        output_last = output[total_seq2seq_rows + sequence_length - horizon, :, :] 
        last_5_pred.append(pred_data_last)
        last_5_output.append(output_last)
        
        total_seq2seq_rows += sequence_length
        
    last_5_pred = np.asarray(last_5_pred, dtype=np.float32)
    last_5_output = np.asarray(last_5_output, dtype=np.float32)

    return last_5_pred, last_5_output 
